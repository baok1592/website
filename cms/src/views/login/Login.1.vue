	...
	<div id="login" v-if="show">
		<div class="login_wrap">
			...
			<form method="post">
				...
				<div>
					<el-row>
						<el-button type="primary" class="btn" @click="sub">登录</el-button>
					</el-row>
				</div>
			</form>
...
<script>
	export default {
		...
		methods: {
			sub() {
				var that = this;
				this.http.post('common/login?ucid=0', { 
					username: that.user,
				...
				.then((res) => {
					...
					localStorage.setItem("region_id", res.data.region_id);
					localStorage.setItem("region_name", res.data.region_name);
					this.$message({
						showClose: true,
						message: '登陆成功',
						type: 'success'
