...
//        return $this->fetch('', ['data' => $data]);
    ...
    public function createData()
    {
        ...
        if ($this->request->isPost()) {
            ...
            // 导入sql数据并创建表
            $vaethink_sql = file_get_contents(VAE_ROOT . '/data/main.sql');
            $sql_array = preg_split("/;[\r\n]+/", str_replace("news_", $data['DB_PREFIX'], $vaethink_sql));
            ...
            $caeate_admin_sql = "INSERT INTO " . "`".$data['DB_PREFIX'] . "admins"."` "
                . "VALUES "
                . "(1, 0, '$username', '超级管理员', '$password', '', '', 0, 0, '超级管理员', 0, $create_time)";
            if (!$link->query($caeate_admin_sql)) {
                throw new BaseException(['msg' => '创建管理员信息失败'.$caeate_admin_sql]);
            }
            $link->commit();
            $link->close();
            $db_str = "
...
default_lang = zh-cn
            ...
            // 创建数据库配置文件
            if (false == file_put_contents(VAE_ROOT . ".env", $env_str)) {
